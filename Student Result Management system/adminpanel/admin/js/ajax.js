...
$(document).on("submit","#adminLoginFrm", function(){
   $.post("query/loginExe.php", $(this).serialize(), function(data){
      if(data.res == "invalid")
      {
        Swal.fire(
          'Invalid  !',
          'Please input valid username / password your username and password is incorrect !,',
          'error'
        )
      }
  ...
  $.post("query/addExamExe.php", $(this).serialize() , function(data){
    ...
    if(data.res == "noDisplayLimit")
   {
    ...
    else if(data.res == "success")
    {
      Swal.fire(
        'Success',
        data.examTitle.toUpperCase() + '<br>Exam Successfully Added !!',
        'success'
      )
          $('#addExamFrm')[0].reset();
          $('#course_name').val("");
...
// delete Examinee
$(document).on("submit","#deleteExamineeFrm" , function(){
  $.post("query/deleteExamineeExe.php", $(this).serialize() , function(data){
     if(data.res == "success")
     {
        Swal.fire(
            'Success',
            data.exFullname + ' <br>has been successfully deleted!',
            'success'
          )
          refreshDiv();
     }
